...
_rule_for(::typeof(q6j), args...) = sixj_sum(canonical_spins(args...)...)
_rule_for(::typeof(fsymbol), args...) = fsymbol_sum(doubled(args...)...)
_rule_for(::typeof(gsymbol), args...) = gsymbol_sum(canonical_spins(args...)...)
...
_admissible_at(::typeof(q6j), k::Int, js...) = _qδtet(canonical_spins(js...)..., k)
_admissible_at(::typeof(gsymbol), k::Int, js...) = _qδtet(canonical_spins(js...)..., k)
...
"""
    ispole_at(k, j1, ..., j6) -> Bool
    ispole_at(symbol, k, labels...) -> Bool

Whether the symbol has a topological pole at level `k`, i.e. some contributing term has negative valuation.
Decided from the valuations alone, with no arithmetic.
"""
ispole_at(k::Integer, args::Spin...) = ispole_at(q6j, k, args...)

function ispole_at(f::Function, k::Integer, args::Spin...)
    k = Int(k)
    _admissible_at(f, k, args...) || return false
    return classify_at_level(_rule_for(f, args...), k)[1] === :pole
end
