...
# ==============================================================================
# 1. NUMERICAL PROJECTORS (Log-Domain Evaluation)
# ==============================================================================
...
"""
    project_classical(dcr::DCR, ::Type{T}=Float64; ws=nothing)
Evaluates the full DCR series as q -> 1. Uses Log-Sum-Exp.
"""
function project_classical(dcr::DCR, ::Type{T}=Float64; ws=nothing) where T
    ...
    max_l = _ws.log_mags[1] = curr_l
    _ws.signs[1] = Int8(curr_s)

    for i in 1:length(dcr.ratios)
        (rl, rs) = _eval_log(dcr.ratios[i])
        curr_l += rl; curr_s *= rs
        _ws.log_mags[i+1], _ws.signs[i+1] = curr_l, Int8(curr_s)
        (curr_l > max_l) && (max_l = curr_l)
    end

    fsum = zero(T)
    @inbounds for i in 1:nt
        fsum += _ws.signs[i] * exp(_ws.log_mags[i] - max_l)
    end
    return exp(max_l) * fsum
end

# ==============================================================================
# 2. EXACT PROJECTORS (Rational BigInt / Zero-GCD)
# ==============================================================================

# Internal helper for exact monomial-to-rational conversion
function _fast_rat_inplace(m::CyclotomicMonomial, sieve::Vector{Int}, buf_big::BigInt)
    ...
    @inbounds for j in eachindex(pe)
        ...
        # In-place GMP power calculation
...
function project_classical_exact(m::CyclotomicMonomial)
    ...
    # The sq_val is the square of the rational value to keep the square root implicit
...
function project_classical_exact(dcr::DCR)
    ...
    curr_exps, min_exps = CLASSICAL_WS_CURR, CLASSICAL_WS_MIN
