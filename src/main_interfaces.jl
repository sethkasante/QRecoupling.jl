...
function qdim(j::Spin, k::OptInt=nothing; mode=nothing, T::Type{<:AbstractFloat}=Float64, prec=256)
    ...
    if !ishalfInt(j) || (2j > k)
        mode == :exact && return ExactResult(k, EMPTY_MONOMIAL, Nemo.QQFieldElem(0))
        return zero(T)
    end
...
function rmatrix(j1::Spin, j2::Spin, j3::Spin, k::OptInt=nothing; 
    ...
    !(abs(j1-j2) <= j3 <= min(j1+j2, k - (j1+j2))) && return (mode == :exact ? ExactResult(k, EMPTY_MONOMIAL, Nemo.QQFieldElem(0)) : zero(T))
