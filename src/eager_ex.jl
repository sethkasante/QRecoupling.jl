# ==============================================================================
# File: evaluation_exact_eager.jl
# Exact SU(2)k Eager Evaluator using Nemo.jl
# ------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------
# 2. The Eager Model (Factorial Cache)
# ------------------------------------------------------------------------------
...
function q6j_exact(j1::Spin, j2::Spin, j3::Spin, j4::Spin, j5::Spin, j6::Spin, k::Int)
    !δtet(j1, j2, j3, j4, j5, j6) && return ExactResult(k, ExactSU2kModel(k).K(1), ExactSU2kModel(k).K(0))
    ...
    for z in z_min:z_max
        ...
        den = model.q_facts[z-α[1]+1] * model.q_facts[z-α[2]+1] * model.q_facts[z-α[3]+1] * model.q_facts[z-α[4]+1] * model.q_facts[β[1]-z+1] * model.q_facts[β[2]-z+1] * model.q_facts[β[3]-z+1]
        term = num * inv(den)
...
function q3j_exact(j1::Spin, j2::Spin, j3::Spin, m1::Spin, m2::Spin, m3::Spin, k::Int)
    (!δ(j1, j2, j3) || m1+m2+m3 != 0) && return ExactResult(k, ExactSU2kModel(k).K(1), ExactSU2kModel(k).K(0))
...
# ------------------------------------------------------------------------------
# 5. Numerical Projection (Evaluation to Float/Complex)
# ------------------------------------------------------------------------------

"""
    evaluate_exact(res::ExactResult, [T=ComplexF64])
Projects the cyclotomic exact result into a complex number using Horner's method.
"""
function evaluate_exact(res::ExactResult, ::Type{T}=ComplexF64; prec=128) where T
    return setprecision(BigFloat, prec) do
        h = res.k + 2
        # target_z = exp(iπ/h)
        target_z = cispi(one(BigFloat) / h)
        
        # Helper: Horner evaluation of nf_elem at complex target_z
        function _horner(poly, z)
            deg = degree(parent(poly))
            val = Complex{BigFloat}(0)
            for i in (deg-1):-1:0
                val = val * z + BigFloat(coeff(poly, i))
            end
            return val
        end
        ...
        # Theory ensures Δ² is real and usually positive in SU(2)k
