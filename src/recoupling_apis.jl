...
"""
    q6j(j1, j2, j3, j4, j5, j6; k=nothing, q=nothing, mode=nothing, exact::Bool=false, eager::Bool=false, T=Float64)
...
- If `mode=:classical`, computes the exact q -> 1 limit.
- If `eager=true`, bypasses DCR construction for raw speed.
"""
function q6j(j1::Spin, j2::Spin, j3::Spin, j4::Spin, j5::Spin, j6::Spin; 
             k=nothing, q=nothing, mode=nothing, exact::Bool=false, eager::Bool=false, T::Type=Float64)
    ...
    if isnothing(k) && isnothing(q) && mode != :classical
        return dcr
    end

    # --- DCR Projections ---
    return project_dcr(dcr; k=k, q=q, mode=mode, exact=exact, T=T)
end


"""
    q3j(j1, j2, j3, m1, m2, m3; k=nothing, q=nothing, mode=nothing, exact::Bool=false, eager::Bool=false, T=Float64)
...
"""
    fsymbol(j1, j2, j3, j4, j5, j6; k=nothing, q=nothing, mode=nothing, exact::Bool=false, T=Float64)
Unitary crossing matrix element: √([d3][d6]) * {6j}.
"""
function fsymbol(j1::Spin, j2::Spin, j3::Spin, j4::Spin, j5::Spin, j6::Spin; 
                 k=nothing, q=nothing, mode=nothing, exact::Bool=false, T::Type=Float64)
...
"""
    gsymbol(j1, j2, j3, j4, j5, j6; k=nothing, q=nothing, mode=nothing, exact::Bool=false, T=Float64)
...
"""
    tetrahedron(j1, j2, j3, j4, j5, j6; k=nothing, q=nothing, mode=nothing, exact::Bool=false, T=Float64)
...
"""
    theta_value(j1, j2, j3; k=nothing, q=nothing, mode=nothing, exact::Bool=false, T=Float64)
...
function theta_value(j1::Spin, j2::Spin, j3::Spin; 
    ...
    # Monomials handle classical logic similarly to DCRs
...
"""
    qdim(j; k=nothing, q=nothing, mode=nothing, exact::Bool=false, T=Float64)
Quantum dimension [2j+1]_q. 
"""
function qdim(j::Spin; k=nothing, q=nothing, mode=nothing, exact::Bool=false, T::Type=Float64)
    mono = qdim_mono(j)
...
"""
    rmatrix(j1, j2, j3; k=nothing, q=nothing, mode=nothing, exact::Bool=false, T=ComplexF64)
Returns the R-matrix phase (braiding eigenvalue) for j1, j2 crossing into j3.
"""
function rmatrix(j1::Spin, j2::Spin, j3::Spin; 
                 k=nothing, q=nothing, mode=nothing, exact::Bool=false, T::Type=ComplexF64)
