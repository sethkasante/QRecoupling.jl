...
export qracah6j, qracah6js, logqnfact_table, logqn_table,
        qδ,
        qdim, 
        SU2kModel,
        rsymbol, 
...
"""
function SU2kModel(k::Int, prec=256)
    tab = get!(LOGQFACT_CACHE, (k, prec)) do
        logqnfact_table(k, prec) 
    end
    return SU2kModel(k, tab)
end
...
"""
function _qracah6j(model::SU2kModel, j1, j2, j3, j4, j5, j6)
...
function _qracah6j_stable(model::SU2kModel, j1, j2, j3, j4, j5, j6)
    if !qδtet(j1, j2, j3, j4, j5, j6, model.k) 
        return zero(BigFloat)
    end
