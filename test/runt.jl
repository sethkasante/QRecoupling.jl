...
@testset "SU2TQFT.jl Master Test Suite" begin
    ...
    @testset "3. Engine Consistency (DCR vs Eager)" begin
        ...
        @test evaluate_exact(ex_eager) ≈ Complex(ex_dcr) atol=1e-15
    end
    ...
    @testset "4. Classical Limits (q -> 1)" begin
        ...
        res_rat = q6j(js..., mode=:classical, eager=true) # Returns ClassicalResult
        val_rat = Float64(res_rat)
        
        # Numerical Log-Sum-Exp Path
        val_num = q6j(js..., mode=:classical, eager=false, T=Float64)
